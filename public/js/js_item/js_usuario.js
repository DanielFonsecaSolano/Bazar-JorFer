...
$(document).ready(function () {
    ...
    $.ajax({
        type: 'POST',
        url: "?controlador=Item&accion=listar_direccion",
        success: function (response) {
            //  console.log(response);
            $.each(JSON.parse(response), function (i, item) {
                $("#direccion_general").append(" <h3 >" + item.direccion_general + "</h3>      <p>" + item.otras_senias + " </p>");
            });
    ...
    $.ajax({
        type: 'POST',
        url: "?controlador=Item&accion=listar_contacto",
        success: function (response) {
            // console.log(response);
            $.each(JSON.parse(response), function (i, item) {
                $("#contacto_local").append(" <h3 >" + item.horario + "</h3>      <p>" + item.numero_telefonico_local + " </p>");
                $("#contacto_local_correo").append(" <h3 >" + item.correo_electronico + "</h3>   <p>¡Envíenos su consulta en cualquier momento!</p> ");
            });
...
var registrar_usuario = function () {
    $("#formularioUsuario").on('submit', function (e) {
        ...
        var id = document.getElementById("id_rol");
        var cedula_usuario = document.getElementById("cedula_usuario");
        ...
        if (cedula_usuario.value === null || cedula_usuario.value === '' || cedula_usuario.value.length < 8) {
            mensaje_error.push('Ingrese una cédula válida');
        }
        if (telefono_usuario.value === null || telefono_usuario.value === '' || telefono_usuario.value.length < 8) {
            mensaje_error.push('Ingrese un teléfono válido');
        }
        if (nombre_usuario.value === null || nombre_usuario.value === '' || nombre_usuario.value.length < 4) {
            mensaje_error.push('Ingrese un nombre válido');
        }
        if (apellido_usuario.value === null || apellido_usuario.value === '' || apellido_usuario.value.length < 8) {
            mensaje_error.push('Ingrese un apellido válido');
        }

        if (direccion_usuario.value === null || direccion_usuario.value === '') {
            mensaje_error.push('Ingrese su dirección');
        }
        ...
        if (mensaje_error.length === 0) {
            ...
            $.ajax({
                type: 'POST',
                url: "?controlador=Item&accion=listar_correo_existente",
                data: {correo: correo_usuario.value},
                cache: false
            })
                    .fail(function () {
                        alert("error");
                    ...
                    .done(function (response) {
                        $.each(JSON.parse(response), function (i, item) {
                            alert("Request: "+item.total);
                            if (item.total >=1) {
                                alert("Este correo electronico, ya ha sido registrado antes.");


                            }  if (item.total== 0) {
                                alert("Correo NO EXISTENTE");
                                 registrar_persona(id.value,cedula_usuario.value,telefono_usuario.value,nombre_usuario.value,
                                 apellido_usuario.value,contrasenia_usuario.value,direccion_usuario.value,
...
var registrar_persona = function (id,cedula,telefono,nombre,apellido,contra,direccion,correo) {
      $.ajax({
                type: 'POST',
                url: "?controlador=Item&accion=registrar_usuario",
                data: {id_rol: id,cedula_usuario:cedula, telefono_usuario:telefono,nombre_usuario:nombre,
                apellido_usuario:apellido,contrasenia_usuario:contra,direccion_usuario:direccion,correo_usuario:correo},
                cache: false
            }).done(function (response) {
                console.log(response);
...
var confirmar_contrasennia = function () {
    $("#confirmar_contrasennia").on('submit', function (e) {
        e.preventDefault();

        var contrasenia_usuario = document.getElementById("contrasenia_usuario");
        var confirmar_contrasenna = document.getElementById("confirmar_contrasenia_usuario");
        var mensaje_error = [];
        var mensaje_confirmacion = [];
        var error = document.getElementById("mensaje");

        if (confirmar_contrasenna.value === contrasenia_usuario.value) {
            mensaje_confirmacion = 'Las contraseñas concuerdan';
        } else {
            mensaje_error = 'Las contraseñas no concuerdan';
        }
        if (confirmar_contrasenna.value === null || confirmar_contrasenna.value === '') {
            mensaje_error = 'Las contraseña inválida';
        }
        if (contrasenia_usuario.value === null || contrasenia_usuario.value === '') {
            mensaje_error = 'Las contraseña inválida';
        }
        ...
        error.innerHTML = mensaje_error + ".";
        error.innerHTML = mensaje_confirmacion + ".";

        if (mensaje_error.length === 0) {
            document.getElementById("confirmar_contrasenia_usuario").value = SHA512(confirmar_contrasenna);
            document.getElementById("contrasenia_usuario").value = SHA512(contrasenia_usuario);

            $.ajax({
                type: 'POST',
                url: "?controlador=Item&accion=generando_contrasennia_usuario",
